...
- 图片读取严格限制在 OB 的 `_media` 持久目录，拒绝路径越界、符号链接、非普通文件、超限文件和伪装成图片的内容。

### 变更 / Changed

- active plan 新增默认 30 天的陈旧确认提醒：旧计划无需迁移，一键 breath 会优先提示待确认计划，Dashboard 可选择继续、完成或放弃；继续、编辑或重新激活只刷新确认时间，系统绝不因超时自动修改计划状态。
