...
- `OMBRE_PRIVATE_CONTINUITY_TOKEN`：私有冲突连续状态同步密钥；未设置时依次回退 `OMBRE_DAILY_NOTE_TOKEN`、`OMBRE_HOOK_TOKEN`。
- `OMBRE_PRIVATE_CONTINUITY_URL`：`sync-private-continuity.py` 使用的完整 `/internal/private-continuity/conflict` 地址，不由 OB 服务端读取。
