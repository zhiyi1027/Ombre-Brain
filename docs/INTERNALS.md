...
- **`breath()`** — 0 参数。调用 `dispatch(startup=True)` 的确定性简报：短核心全文、最近 24 小时中最新/重要的最多 3 条正文、最多 1 条较早未完正文、最多 5 条 active plan 简表；日常每次对话开头调用。
...
3. **浮现模式**（无 `query`）：无参 `breath()` 启用 `startup=True`，转入独立的确定性简报算法：钉选短核心逐字返回；最近 24 小时最新一条固定入选，其余按 importance、created、score 稳定排序，最近正文最多 3 条；再从 24 小时之外取 score 最高的一条未解决正文；active plan 按 weight/created 稳定排序列最多 5 条简表。测试数据、anchor、dont_surface 与私有类型不会进入最近/未完池。普通低优先级正文受 `startup_breath_soft_tokens` 软目标约束，最新记忆或 importance≥8 的正文可延伸到 `startup_breath_max_tokens` 硬上限；超预算时只列名称、bucket_id 与整桶 token 估算，绝不截断。启动模式不调用随机数、不追加久未浮现或 3% 偶遇，也不调用 `touch()`。`breath_advanced()` 无 query 时保留原完整模式：未解决桶按衰减分排序，支持加权采样或 Top-1 + shuffle，并可追加久未浮现与偶遇。
...
| `surfacing.startup_breath_soft_tokens` | `3000` | 无参 `breath()` 的普通输出目标（500-10000）；低优先级正文超过时只列索引 |
