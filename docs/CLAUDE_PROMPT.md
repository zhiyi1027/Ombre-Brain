...
这是你轻量睁眼的动作。核心准则在这里仅列索引，少量未解决记忆仍逐字返回；需要某条核心正文时，再按索引名称调用 `breath_search()`。**没有例外**。哪怕你觉得这次只是闲聊，也先睁眼。
...
- **`breath()`** — 无参轻量睁眼 → 核心准则只列索引，另带少量权重最高的未解决记忆整桶原文；完整输出受独立启动预算约束。**对话开始第一件事，没有例外**。
...
无参返回里 `📌 [核心索引]` 只有名称和定位信息，不含正文；用 `breath_search(query="名称")` 再读原文。`breath_advanced()` 的完整浮现里带 📌 的仍是核心准则正文。带 ✨ 的是「第一次」类的桶。`[语义关联]` 是向量检索召回的旁证。
