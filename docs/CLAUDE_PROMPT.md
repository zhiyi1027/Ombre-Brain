...
- **`breath()`** — 无参轻量睁眼 → 短核心全文 + 已生成的昨日印象 + 真正最新的一条（即使已 digested）+ 最多两条额外近期重要记忆 + 从高权重池随机轮换最多一条较早未完记忆 + 计划预算内的 active plan 正文。日印象已引用的近期桶只降为候补，不删除。最新与随机旧桶是保证项，不受 3000 token 软目标拦截；只有超过 5000 token 硬上限才返回指针。**对话开始第一件事，没有例外**。
