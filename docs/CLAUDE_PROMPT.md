...
这是你的一键睁眼简报。它完整返回短核心、昨日印象、近期交接、最多两条尚未消化的自动精读记忆、一条有门槛的旧事联想、活动计划，以及与本轮完整记忆相关的最多五条 feel。**没有例外**。哪怕你觉得这次只是闲聊，也先睁眼；一次调用完成后即可开口，不必再固定调用 `dream(catalog=True)`、`breath_search()` 或 feel 通道。
...
- **`breath()`** — 无参一键睁眼 → 短核心全文 + 已生成的昨日印象 + 真正最新的一条（即使已 digested）+ 最多两条额外近期重要记忆 + 最多一条有门槛的旧事联想 + 计划预算内的 active plan 正文；随后自动精读最近 48 小时中尚未消化、未解决且没有在前述正文出现的最多 2 桶，再按这些完整普通记忆附上最多 5 条相关 feel（直属 `source_bucket` 优先，语义＋关键词补足）。日印象已引用的近期桶只降为候补，不删除。基础记忆默认硬上限 5000，自动精读与相关 feel 各有 2000 独立预算，默认总硬上限 9000；正文只整桶返回。**对话开始第一件事，没有例外；一次调用后即可开口。**
