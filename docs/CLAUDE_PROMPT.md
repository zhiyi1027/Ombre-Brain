...
- **`breath()`** — 无参轻量睁眼 → 短核心全文 + 已生成的昨日印象 + 真正最新的一条（即使已 digested）+ 最多两条额外近期重要记忆 + 最多一条有门槛的旧事联想 + 计划预算内的 active plan 正文。日印象已引用的近期桶只降为候补，不删除。旧事只从“importance≥8 且从未激活”或“importance≥9 且至少 7 天未活跃”的较早未完桶中随机轮换，并避免紧邻重复。固定记忆槽不受 3000 token 软参考拦截；只有超过 5000 token 硬上限才返回指针。**对话开始第一件事，没有例外**。
