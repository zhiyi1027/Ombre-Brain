...
async def breath_search(
    ...
    """按关键词/语义检索记忆桶,融合关键词/BM25+语义检索,向量不可用时明确提示并退回关键词检索。命中后逐字返回桶内当前 content，不调用 LLM 摘要/改写；非删除 archived 桶可由关键词或完整 bucket ID 找回并标记 archived:true，软删除/tombstone 始终隐藏。domain 逗号分隔,按主题域预筛。max_results=返回条数上限(默认 5,最大 50)；需要更多结果或 tags/importance_min/valence/arousal/max_tokens/catalog 等过滤维度时用 breath_advanced(...)。quotes=True 只在已经命中一条记忆后附上当初主动保留的原话；默认不返回，也不能用于列出全部引语。"""
...
async def breath_advanced(
    ...
    """breath 的完整参数版,给需要精细控制的场景用(日常用 breath()/breath_search() 就够了)。不传 query=返回权重最高的未解决记忆;传 query=融合关键词/BM25+语义检索，向量不可用时明确提示并退回关键词检索。命中后逐字返回桶内当前 content，不调用 LLM 摘要/改写；非删除 archived 桶可由关键词或完整 bucket ID 找回并标记 archived:true，软删除/tombstone 始终隐藏；max_tokens 不足时整桶省略，绝不截断正文。catalog=True=目录模式:只返回每桶一行元数据(名称|域|重要度,0 LLM 调用,最省 token),适合开新对话先看目录再 breath_search(query=...) 精准拉取,可配 domain 过滤。max_tokens=单次返回总 token 上限(默认 config.surfacing.breath_max_tokens,fallback 10000)。domain=\"plan\" 且不传 query 时逐字返回 active plans；domain=\"feel\" 必须带 query，先返回 query 中 bucket_id/source_bucket 直属的 feel，再按语义+关键词补足，最多 5 条，不再全量返回。domain 其余值按主题域预筛,valence/arousal 0~1(-1 忽略)。max_results=返回条数上限(默认 config.surfacing.breath_max_results,fallback 20,最大 50；feel 通道硬上限 5)。importance_min>=1=跳过语义检索,按重要度降序返回最多 20 条高重要度记忆。tags 逗号分隔,AND 过滤;tags=\"feel\" 或 \"__feel__\" 等价于 domain=\"feel\"，同样必须带 query。"""
    return await _with_notice(
        _t_breath.dispatch(
            query=query, max_tokens=max_tokens, domain=domain,
            valence=valence, arousal=arousal, max_results=max_results,
...
async def plan(
    ...
    """登记一个待办/承诺/未闭环事项。status=active(默认)/resolved/abandoned。related_bucket 可选,关联到某个普通记忆桶。weight=承诺重量 0.0-1.0(默认 0.5),与 importance 区分——importance 表示「多重要」、weight 表示「多重」。why_remembered=登记原因(可选、仅展示)。plan 不衰减,在一键 breath 的独立计划段和 dream 末尾返回；长期未确认时只提醒,绝不自动改状态,可在 Dashboard 继续/完成/放弃。后续 hold/grow 写入新事件时系统自动判断已登记的 plan 是否完成。"""
    return await _with_notice(
        _t_plan.plan_create(
